const ROLES = ["superadmin", "admin", "editor", "auditor"];

// superadmin: everything. editor: full case/identity control, no user admin.
// admin: user management only. auditor: read-only.
...
function assignableRoles(actorRole) {
	if (actorRole === "superadmin") return ["admin", "editor", "auditor"];
	if (actorRole === "admin") return ["editor", "auditor"];
	return [];
}

// Nobody manages the superadmin; admins are limited to editor/auditor targets.
function canActOnUserRole(actorRole, targetRole) {
	if (targetRole === "superadmin") return false;
	if (actorRole === "superadmin") return true;
	if (actorRole === "admin") return targetRole === "editor" || targetRole === "auditor";
	return false;
}
